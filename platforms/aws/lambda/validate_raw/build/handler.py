...
# expected columns per domain table
SCHEMA_RULES = {
    "claims": {
        "required_columns": [
...
def get_table_name(s3_key: str) -> str:
    ...
    parts = s3_key.split("/")
    # path: dev/domain/table_name/YYYY/MM/DD/HHmmss/file.parquet
    return parts[2] if len(parts) > 2 else "unknown"


def validate_parquet(bucket: str, key: str) -> dict:
    """Download parquet file from S3 and validate schema and row count."""
